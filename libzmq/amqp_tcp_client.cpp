...
void zmq::amqp_tcp_client_t::send_to (const char *exchange_, pipe_t *pipe_)
{
...
void zmq::amqp_tcp_client_t::receive_from (const char *queue_, pipe_t *pipe_)
{
    //  Start receiving messages from a pipe.
    mux.receive_from (pipe_, state == state_shutting_down);
    if (state != state_connecting && state != state_shutting_down)
        poller->set_pollout (handle);
}

void zmq::amqp_tcp_client_t::terminate_pipe (pipe_t *pipe_)
{
...
void zmq::amqp_tcp_client_t::terminate_pipe_ack (pipe_t *pipe_)
{
    //  Forward the command to the pipe.
