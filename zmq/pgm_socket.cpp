...
zmq::pgm_socket_t::pgm_socket_t (bool receiver_, bool passive_, const char *iface_,
    const char *mcast_group_, uint16_t port_, size_t readbuf_size_): g_transport (NULL), 
    pgm_msgv (NULL), pgm_msgv_len (-1)
{
    ...
    //  Create TIBCO Rendezvous compatible network parameter string
    char network [128];
    snprintf (network, sizeof (network), "%s;%s", iface_, mcast_group_);
    zmq_log (4, "network %s\n", network);

    //  Store socket info
    socket_info.assign (mcast_group_);
    socket_info += ":";
    std::stringstream sstream;
    sstream << port_;
    socket_info.append (sstream.str ());
    zmq_log (4, "socket_info %s\n", socket_info.c_str ());

    rc = pgm_if_parse_transport (network, AF_INET, &recv_gsr, &send_gsr, &gsr_len);
    assert (rc == 0);
    assert (gsr_len == 1);

    rc = pgm_transport_create (&g_transport, &gsi, port_, &recv_gsr, 1, &send_gsr);
    errno_assert (rc == 0);
