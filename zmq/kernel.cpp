...
zmq::kernel_t::kernel_t (bool listen, const char *address, unsigned short port,
      bool xmode) :
    xmode (xmode),
    xmode_blocked (!xmode),
    peer_dead (false),
...
bool zmq::kernel_t::send (void *data, size_t size, free_fn *ffn)
{
...
bool zmq::kernel_t::receive (void **data, size_t *size, free_fn **ffn)
{
    //  If the other party have closed the socket, we cannot read messages.
    if (peer_dead)
        return false;
...
void *zmq::kernel_t::worker_routine (void *arg)
{
...
bool zmq::kernel_t::flush_sendbuf ()
{
...
bool zmq::kernel_t::fill_msg (msg_t &msg)
{
...
void zmq::kernel_t::do_work ()
{
    ...
    while (true) {
        ...
        //  for errors.
        pfd [1].events = (pollin ? POLLIN : 0) | (pollout ? POLLOUT : 0);
        int rc = poll (pfd, 2, -1);
        errno_assert (rc != -1);
        assert ((pfd [0].revents & (POLLERR | POLLHUP | POLLNVAL)) == 0);
        assert ((pfd [1].revents & POLLNVAL) == 0);

        //  Command from client thread was received by the worker thread.
